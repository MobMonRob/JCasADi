...
run() {
	...
	# Needed for cross compilation of some plugins. (Those without CMAKE_ARGS)
	export CROSS_TRIPLE=x86_64-w64-mingw32

	#Um rdynamic Fehler mit mingw bei denjenigen Plugins zu verhindern, für die CMAKE_ARGS in der CMakeLists.txt von CasaDi verwendet wird:
	## CMAKE_ARGS hinzufügen: -DCMAKE_TOOLCHAIN_FILE=${CMAKE_CURRENT_LIST_DIR}/../../../../mingw.cmake
	## Bei neuer Version: anpassen in windows_casadi.patch
	...
	cmake -DCMAKE_SYSTEM_NAME="Windows" -DCMAKE_CROSS_COMPILING=ON -DCMAKE_TOOLCHAIN_FILE="$scriptDir/mingw.cmake" -DCMAKE_BUILD_TYPE="Dummy" "$scriptDir/$casadiDir" -DWITH_EXTRA_CHECKS=OFF -DWITH_SO_VERSION=OFF -DWITH_SELFCONTAINED=ON -DWITH_EXAMPLES=OFF -DCMAKE_INSTALL_PREFIX="$localTarget2" -DWITH_BUILD_REQUIRED=OFF -DWITH_BUILD_SUNDIALS=OFF -DWITH_SUNDIALS=OFF -DWITH_BUILD_CSPARSE=OFF -DWITH_CSPARSE=OFF -DWITH_BUILD_TINYXML=OFF -DWITH_TINYXML=OFF -DWITH_MOCKUP_REQUIRED=OFF -DWITH_MUMPS=ON -DWITH_BUILD_MUMPS=ON -DWITH_METIS=ON -DWITH_BUILD_METIS=ON -DWITH_LAPACK=ON -DWITH_BUILD_LAPACK=ON
