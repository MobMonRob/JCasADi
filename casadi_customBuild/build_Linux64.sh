...
run() {
	...
	# Für Release: (Ohne)
	cmake -DCMAKE_CXX_FLAGS:STRING="-O0" -DCMAKE_BUILD_TYPE="Debug" -DCMAKE_INSTALL_PREFIX="$localTarget2" -DWITH_BUILD_SUNDIALS=OFF -DWITH_SUNDIALS=OFF -DWITH_BUILD_CSPARSE=OFF -DWITH_CSPARSE=OFF -DWITH_BUILD_TINYXML=OFF -DWITH_TINYXML=OFF -DWITH_MOCKUP_REQUIRED=OFF -DWITH_EXAMPLES=OFF "$scriptDir/$casadiDir"
