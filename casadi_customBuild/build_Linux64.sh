#!/bin/bash

scriptPath="$(realpath -s "${BASH_SOURCE[0]}")"
scriptDir="$(dirname "$scriptPath")"
cd "$scriptDir"

source "./_bash_config.sh"

run() {
	setCurrentPlatform "$platformLinux"

	local -r localTarget2="$scriptDir/$currentTarget"

	local -r tmpDir="./$currentTmp/build"
	rm -rdf "$tmpDir"
	mkdir -p "$tmpDir"

	cd "$tmpDir"

	export LD_RUN_PATH="\$ORIGIN"

	# Zum Testen des Bauens nur (Schneller Build, Langsame Ausführung) (5 min): -DCMAKE_CXX_FLAGS:STRING="-O0" -DCMAKE_BUILD_TYPE="Debug"
	# Für Release: (Ohne)
	cmake -DWITH_SELFCONTAINED=ON -DCMAKE_INSTALL_PREFIX="$localTarget2" -DWITH_BUILD_REQUIRED=ON -DWITH_IPOPT=ON -DWITH_BUILD_IPOPT=ON -DWITH_MUMPS=ON -DWITH_BUILD_MUMPS=ON -DWITH_METIS=ON -DWITH_BUILD_METIS=ON -DWITH_BUILD_SUNDIALS=OFF -DWITH_SUNDIALS=OFF -DWITH_BUILD_CSPARSE=OFF -DWITH_CSPARSE=OFF -DWITH_BUILD_TINYXML=OFF -DWITH_TINYXML=OFF -DWITH_MOCKUP_REQUIRED=OFF -DWITH_EXAMPLES=OFF "$scriptDir/$casadiDir"
	#/home/fabian/Desktop/minimal_cmake_example/

	time nice -n19 ionice -c2 -n7 make --jobs="$((1+$(nproc)))"

	rm -rdf "$localTarget2/casadi"
	mkdir -p "$localTarget2"
	make install

	cd "$scriptDir"
}

run_bash run $@

