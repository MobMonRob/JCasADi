...
public class Nlpsol {
    ...
    static MX p = MxStatic.sym("p", 2);
    static MX v = MxStatic.sym("v", 2);
    static MX states = MxStatic.vertcat(new StdVectorMX(new MX[]{p, v}));
    static MX speed = MxStatic.norm_2(v);
    static MX rhs = MxStatic.vertcat(new StdVectorMX(new MX[]{v, MxStatic.times(MxStatic.times(new MX(-c), speed), MxStatic.rdivide(v, new MX(m)))}));
    static MX T = MxStatic.sym("T");
    static MX X0 = MxStatic.sym("X0", 4);

    static MX theta = MxStatic.sym("theta");
    static MX v1 = MxStatic.sym("v");
    static MX theta_rad = MxStatic.times(MxStatic.rdivide(theta, new MX(180)), new MX(Math.PI));
    ...
    public static Function fly2_2() {
        ...
        ode_T.put("x", states);
        ode_T.put("ode", MxStatic.times(T, rhs));
        ode_T.put("p", T);
    ...
    public static Function shoot_distance2_5() {
        ...
        var height = MxStatic.vertcat(result).at(1); // -> wrong size, see below/above
        ...
        inputMap.put("x", T);
        inputMap.put("p", MxStatic.vertcat(new StdVectorMX(new MX[]{v1, theta})));
        ...
        outputMap.put("p", MxStatic.vertcat(new StdVectorMX(new MX[]{v1, theta})));
    ...
    public static void exercise_presentation() {
        ...
        MX z = MxStatic.sym("z");
        MX[] v = new MX[]{x, y, z};
        MX f = MxStatic.plus(MxStatic.pow(x, new MX(2)), MxStatic.times(new MX(100), MxStatic.pow(z, new MX(2))));
        MX g = MxStatic.minus(MxStatic.plus(z, MxStatic.pow(MxStatic.minus(new MX(1), x), new MX(2))), y);
        Map<String, MX> inputMap = new StdMapStringToMX();
        inputMap.put("x", MxStatic.vertcat(new StdVectorMX(v)));
        inputMap.put("f", f);
        inputMap.put("g", g);
        StdMapStringToMX mp = new StdMapStringToMX(inputMap);
        var solver = nlpsol("solver", "qrsqp", mp);
        inputMap.clear();
        inputMap.put("x0", MxStatic.vertcat(new StdVectorMX(new MX[]{new MX(2.5), new MX(3.0), new MX(0.75)})));
    ...
    public static void exercise1_2() {
        ...
        outputMap.put("x0", DmStatic.vertcat(new StdVectorDM(new DM[]{new DM(0), new DM(0), new DM(35), new DM(30)})));
    ...
    public static void exercise2_4() {
        ...
        var height = MxStatic.vertcat(result).at(1); // -> wrong size, see below
        // corresponding python-code: height = x[1]
    ...
    public static void exercise3_1() {
        ...
        nlp.put("x", theta);
        nlp.put("f", MxStatic.times(new MX(-1), MxStatic.vertcat(result_sd)));
    ...
    public static void exercise3_2() {
        ...
        var J = MxStatic.jacobian(MxStatic.vertcat(result), MxStatic.vertcat(new StdVectorMX(new MX[]{v1, theta})));

        var sigma_shoot_distance = mtimes(mtimes(J, cov_vtheta), J.T());

        StdMapStringToMX nlp = new StdMapStringToMX();
        nlp.put("x", MxStatic.vertcat(new StdVectorMX(new MX[]{v1, theta})));
        nlp.put("g", MxStatic.vertcat(result));
        nlp.put("f", sigma_shoot_distance);

        var solver = nlpsol("solver", "ipopt", nlp);

        StdMapStringToDM outputMap = new StdMapStringToDM();
        outputMap.put("x0", DmStatic.vertcat(new StdVectorDM(new DM[]{new DM(30), new DM(45)})));
