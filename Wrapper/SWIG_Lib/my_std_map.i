// Based on SWIG's std_map.i with last commit from 11.10.2023.
...
%}
  ...
  public:
    ...
    struct iterator {
      // Conflicts with %feature("nspace");
      // %typemap(javaclassmodifiers) iterator "protected class"
      %extend {
        std::map< K, T >::iterator getNextUnchecked() {
          std::map< K, T >::iterator copy = (*$self);
          return ++copy;
        }
...
%#endif
      ...
      void removeUnchecked(const std::map< K, T >::iterator itr) {
        self->erase(itr);
      }
    }
};
